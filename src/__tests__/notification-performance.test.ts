...
describe('通知システム パフォーマンステスト', () => {
  ...
  it('メモリ使用量が適切に管理される', () => {
    ...
    console.log(`メモリ使用量変化: +${(memoryIncrease / 1024 / 1024).toFixed(2)}MB → -${(memoryDecrease / 1024 / 1024).toFixed(2)}MB`);
  ...
  it('通知データのシリアライゼーションが効率的', () => {
    const complexPayload: NotificationPayload = {
      ...
      data: {
        ...
        statistics: {
          totalCount: 100,
          averageScore: 75.5,
          distribution: Array.from({ length: 10 }, (_, i) => ({ range: i * 10, count: Math.floor(Math.random() * 20) }))
        }
      }
    };
    ...
    expect(duration).toBeLessThan(50); // 50ms以内で完了
    console.log(`100回の複雑データシリアライゼーション: ${duration.toFixed(2)}ms`);
  });
});
...
describe('通知システム 品質メトリクス', () => {
  ...
  it('エラー境界が適切に処理される', async () => {
    ...
    const payload = service.createHighScoreNotification('テスト', 100, 'テスト');
  ...
  it('入力値の妥当性チェックが機能する', () => {
    // 空文字やnullの処理
    expect(() => service.createHighScoreNotification('', 0, '')).not.toThrow();
    expect(() => service.createWeatherAlertNotification('', '')).not.toThrow();
    expect(() => service.createRegularReportNotification('')).not.toThrow();

    // 極端な値の処理
    expect(() => service.createHighScoreNotification('a'.repeat(1000), -999, 'b'.repeat(1000))).not.toThrow();
    expect(() => service.createHighScoreNotification('テスト', Number.MAX_SAFE_INTEGER, 'テスト')).not.toThrow();
  });
  ...
  it('国際化対応の基盤が整っている', () => {
    ...
    const payload = service.createHighScoreNotification('Tennis', 85, 'Sunny');
