...
export class NotificationConfigService {
    ...
    async getNotificationConfigsByType(
        type: NotificationType
    ): Promise<NotificationConfig[]> {
        return await this.db.notificationConfigs
            .where('type')
            .equals(type)
            .toArray();
    }
    ...
    async addNotificationHistory(
        ...
        return await this.db.notificationHistory.add(
            history as NotificationHistory
        );
    }

    async getNotificationHistorySimple(
        limit?: number
    ...
    async getNotificationHistoryByType(
        type: NotificationType,
        limit?: number
    ): Promise<NotificationHistory[]> {
    ...
    async getNotificationHistoryByConfig(
        configId: number,
        limit?: number
    ): Promise<NotificationHistory[]> {
        const query = this.db.notificationHistory
    ...
    // 通知可能な時間帯かチェック
    async isNotificationTimeAllowed(
    ...
    async getNotificationHistory(filter?: {
        ...
        const db = dbService.getDb(); // Access the database directly from the service
        ...
        let query;
        if (filter?.since) {
            query = db.notificationHistory.where('sentAt').above(filter.since);
        } else {
            query = db.notificationHistory.orderBy('sentAt').reverse();
        }

        if (filter?.type) {
            query = query.and((h) => h.type === filter.type);
        }
