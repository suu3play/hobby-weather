...
export class RegularReportNotificationService {
  ...
  private generateSummary(recommendations: HobbyRecommendation[], period: ReportPeriod): string {
    ...
    if (highScoreCount === 1) {
      return `${periodText[period.type]}は${topHobby.hobby.name}が特に最適です（スコア${Math.round(topHobby.overallScore)}点）。`;
    }
  ...
  private generateWeatherSummary(forecast: WeatherForecast, period: ReportPeriod): string {
    ...
    let summary = `現在は${weatherDescription}で気温${Math.round(currentWeather.temperature)}°C、`;
    
    if (currentWeather.precipitationProbability > 60) {
      summary += `降水確率${currentWeather.precipitationProbability}%と雨の可能性が高めです。`;
    } else if (currentWeather.precipitationProbability > 30) {
      summary += `降水確率${currentWeather.precipitationProbability}%と変化しやすい天候です。`;
    } else {
      summary += `降水確率${currentWeather.precipitationProbability}%と安定した天候です。`;
    }
  ...
  private generateActionItems(
    ...
    if (highScoreRecs.length > 0) {
      ...
      actions.push(`${topHobby.hobby.name}を今日実行するのに最適です`);
    }
    ...
    } else if (current.precipitationProbability > 30) {
  ...
  async getRegularReportStatistics(): Promise<{
    ...
    try {
      ...
      const scores = history
        .map(h => h.data?.topHobbies?.[0]?.score || 0)
      ...
      history.forEach(h => {
        if (h.data?.topHobbies) {
          h.data.topHobbies.forEach((hobby: any) => {
      ...
      return {
        ...
        averageScore,
        mostFrequentHobby,
        lastReportTime: history.length > 0 ? history[0].sentAt : null
      };
