...
export class NotificationSchedulerService {
  ...
  private regularReportService = RegularReportNotificationService.getInstance();
  ...
  async scheduleConfigTasks(config: NotificationConfig): Promise<void> {
    ...
    for (const nextRun of nextRuns.slice(0, 3)) { // 最大3つまで
  ...
  private async executeTask(task: ScheduledTask): Promise<void> {
    try {
      ...
      // 通知可能かチェック
      const canNotify = await this.configService.isNotificationTimeAllowed(task.config);
      const hasReachedLimit = await this.configService.hasReachedDailyLimit(task.config.type);

      if (!canNotify || hasReachedLimit) {
        console.log(`通知スキップ: ${task.config.title} (時間外またはレート制限)`);
      } else {
        // 通知を送信
        const success = await this.notificationService.sendNotification(task.payload);
  ...
  // 次回実行時刻の計算
  private calculateNextRuns(config: NotificationConfig, fromDate?: Date): Date[] {
    ...
    switch (schedule.frequency) {
      ...
      case 'daily':
        ...
        for (const timeRange of schedule.timeOfDay) {
          const nextRun = this.getNextDailyRun(now, timeRange.start, schedule.daysOfWeek);
      ...
      case 'weekly':
        ...
        for (const dayOfWeek of schedule.daysOfWeek) {
          for (const timeRange of schedule.timeOfDay) {
            const nextRun = this.getNextWeeklyRun(now, dayOfWeek, timeRange.start);
            if (nextRun) nextRuns.push(nextRun);
          }
        }
        break;
  ...
  // 毎日の次回実行時刻を計算
  private getNextDailyRun(from: Date, time: string, allowedDays: number[]): Date | null {
  ...
  // 週次の次回実行時刻を計算
  private getNextWeeklyRun(from: Date, dayOfWeek: number, time: string): Date | null {
    const [hours, minutes] = time.split(':').map(Number);
    const nextRun = new Date(from);
  ...
  // 設定に応じたペイロード作成
  private async createPayloadForConfig(config: NotificationConfig): Promise<NotificationPayload> {
    // 基本的なペイロード
    ...
    switch (config.type) {
      case 'high-score':
        // 高スコア通知の動的評価
        const highScoreResult = await this.highScoreService.evaluateAndCreateNotification();
        if (highScoreResult.notificationSent && highScoreResult.recommendations.length > 0) {
          const recommendations = highScoreResult.recommendations.map(rec => ({
          ...
          try {
            ...
            const forecast = await new weatherService.WeatherService().getCurrentForecast();
            if (forecast) {
              const weatherDescription = this.getWeatherDescription(forecast.current.weatherType);
              const temperature = Math.round(forecast.current.temperature);
              
              payload = this.notificationService.createDetailedHighScoreNotification(
                recommendations,
                weatherDescription,
                temperature
              );
              ...
              payload = this.notificationService.createHighScoreNotification(
                recommendations[0].hobbyName,
                recommendations[0].score,
                '現在の天気'
              );
            }
          } catch (error) {
            console.error('天気情報の取得に失敗、フォールバック通知を使用:', error);
            payload = this.notificationService.createHighScoreNotification(
              '趣味活動',
              85,
              '現在の天気条件'
          ...
          payload = {
            type: 'high-score',
            title: '趣味チェック完了',
            message: highScoreResult.reason || '現在、高スコアの趣味はありません'
          };
        }
        break;
      ...
      case 'weather-alert':
        // 天気急変アラートの動的評価
        const weatherAlerts = await this.weatherAlertService.evaluateWeatherAlerts();
        const activeAlert = weatherAlerts.find(alert => alert.alertTriggered);
        
        if (activeAlert) {
          payload = this.notificationService.createDetailedWeatherAlertNotification(
      ...
      case 'regular-report':
        // 定期レポートの動的生成
        const reportResult = await this.regularReportService.generateRegularReport();
        
        if (reportResult.reportGenerated && reportResult.reportContent) {
          const content = reportResult.reportContent;
          const topHobbies = content.topRecommendations.slice(0, 3).map(rec => ({
            name: rec.hobby.name,
            score: rec.overallScore
          }));
          
          payload = this.notificationService.createDetailedRegularReportNotification(
            content.summary,
            topHobbies,
          ...
          payload = {
            type: 'regular-report',
            title: '📊 趣味レポート',
            message: reportResult.reason || 'レポートを生成できませんでした'
          };
        }
        break;
    }
