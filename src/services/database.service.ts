...
import type { Hobby, WeatherData, WeatherForecast, Location, AppSettings } from '../types';
...
export class DatabaseService {
  ...
  // Hobby operations
  async createHobby(hobby: Omit<Hobby, 'id' | 'createdAt' | 'updatedAt'>): Promise<number> {
  ...
  async getActiveHobbies(): Promise<Hobby[]> {
    return await this.db.hobbies.filter(hobby => hobby.isActive === true).toArray();
  }
  ...
  async saveWeatherData(weatherData: Omit<WeatherData, 'id'>): Promise<number> {
    return await this.db.weatherData.add(weatherData as WeatherData);
  }

  async getWeatherData(lat: number, lon: number, maxAge: number = 6): Promise<WeatherData | undefined> {
  ...
  async saveWeatherForecast(forecast: Omit<WeatherForecast, 'id'>): Promise<number> {
    // Delete existing forecast for same location
    await this.db.weatherForecasts.where('[lat+lon]').equals([forecast.lat, forecast.lon]).delete();
    return await this.db.weatherForecasts.add(forecast as WeatherForecast);
  }

  async getWeatherForecast(lat: number, lon: number, maxAge: number = 6): Promise<WeatherForecast | undefined> {
    const cutoffTime = new Date(Date.now() - maxAge * 60 * 60 * 1000);
  ...
  // Location operations
  async saveLocation(location: Omit<Location, 'id' | 'createdAt'>): Promise<number> {
    if (location.isDefault) {
      // Set all other locations as non-default
      await this.db.locations.filter(loc => loc.isDefault === true).modify({ isDefault: false });
    }
  ...
  async getDefaultLocation(): Promise<Location | undefined> {
    return await this.db.locations.filter(loc => loc.isDefault === true).first();
  }
